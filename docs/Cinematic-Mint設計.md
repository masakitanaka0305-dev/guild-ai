...
| Phase 3 — 静謐の間 | curtain — 啓示の前の "静かな闇"（#129 強化） | 1.4s | 0.0–0.4s curtain fade（`#020617`）→ 0.4–1.0s **3 つのドット「…」が purple `#4C1D95` で 1 個ずつ呼吸**＋上下にゴールドの hairline がそれぞれ伸びる（"タメ"）→ 1.0–1.4s hairline が中央 1 点に **convergence** して Phase 4 の radial-glow に繋がる。`role="status" aria-live="polite"` で「準備中…」を 1 度だけ通知。**エラー風／クラッシュ風／loader spinner は一切無し** |
| Phase 4 — 啓示 | 結果の浮上（#130 ランク連動） | 1.0s | 黒からゴールドの radial-glow（**S=380px / A=320px / B=260px / D=200px**）で中央に **HexRankBadge 80px ＋ ランク連動光**。`metric-hero` で **資産価値：¥XXX,XXX** を tabular-nums で表示。**S だけ Star 粒子 6 個**を周囲に静的スプレッド（A は 2 個 / B,D は 0 個）。サブに「あなたの知恵が、銀行に届きました」＋ **希少度キャプション**（getRarity(rank).caption — 例「この太鼓判は希少です（直近 100 件で 8%）」）。続いて 2 つのアクション「マイ銀行で確認 / もう一度出品する」 |

合計尺 = 1400 + 1400 + 1400 + 1000 = **5200 ms** ≤ 5.4 s（Phase 3 を #129 で 0.8s → 1.4s に延長、`PHASE3_BEATS = { fadeMs: 400, tameMs: 600, convergeMs: 400 }`）。テストで上限ロック。
...
| `curtain-fade` | 400ms | Phase 3 の暗転（最初の 0.4s でゆるく opacity 1 へ） |
| `ellipsis-pulse` | 900ms infinite | Phase 3 中央の 3 ドットが順繰りに呼吸（200ms 位相差） |
| `hairline-grow` | 600ms | Phase 3 上下のゴールド hairline が伸びる |
| `hairline-converge` | 400ms | Phase 3 終端で hairline が 1 点に集約（Phase 4 への hand-off） |
