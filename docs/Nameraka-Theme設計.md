...
1. **ヒーロー**：「AIエージェントで、あなたの時間をアップデート。」（h1） + 「日本最大のAIエージェント・プラットフォーム」 + 2 CTA
2. **3 アクションタイル**：📝のこす（30秒）／💼かせぐ（即決）／🏦ふえる（24時間）— 3 カラム、説明文なし、ピクト 36px + 単語 18px
3. **いま のこされた しごと**：横スクロール 3 件、タイトル＋¥価格のみ（説明文なし）
4. **かせげる しごと**：縦リスト 2 件、タイトル＋報酬＋「応募する」赤ボタン
5. **帯バナー**：「まだ かせいでない？」右に「いま のこす」赤丸ボタン、背景グレー・テキストなし
6. **フッタ**：GUILD AI ロゴ＋ナビリンク（コピーテキスト最小）
