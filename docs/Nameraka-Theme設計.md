...
- **ヒーローブロック**（最上部）：「マイ銀行：あなたの報酬と資産をまとめる場所」＋サブ文（「今日の報酬」「今月の合計」「推定時給」を赤強調）
...
- **運用中の資産：あなたが投稿したMDファイル**（`<SectionBand>` + `<Tip>`、`<AssetPortfolio>` コンポーネント）
  ...
  - 「＋ 新しく投稿する」→ `/sell`
  - 各資産：タイトル・ステータスバッジ（運用中 緑 / 審査中 黄 / 停止中 グレー）・公開エンドポイント短縮・今月の報酬・累計コール数 Sparkline・最終呼び出し相対時刻・「詳細」→ `/asset/{guildId}`
