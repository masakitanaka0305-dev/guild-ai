...
- **ヒーローブロック**（最上部）：「マイ銀行：あなたの "おだちん" を まとめる場所」＋サブ文（「今日の おだちん」「今月の合計」「いまの推定時給」を赤強調）
- **「いまの推定時給」カード**：大型数値 `¥X / 時間`、緑パルス SVG、`aria-live="polite"`、`<Tip>` ？アイコン：「直近60秒の API 印税×60 で計算しています」
- 2 数字カード（今日のおだちん / 今月の合計）
- **稼働中ノート上位 3 件**（`<SectionBand>` 見出し帯 + `<Tip>`）：直近1分コール数・単価・累計売上、横スクロール（PC縦リスト）
- **通帳：これまでの お取引**（`<SectionBand>` 見出し帯 + `<Tip>`）：入出金テーブル（おだちん＋印税行）
...
| ギルド通帳 | 通帳（セクション名） |
| お財布 | マイ銀行 または 通帳 |
| おさいふ通帳（/wallet nav ラベル） | おさいふ通帳（/wallet 専用ページラベルのみ残存）|

- **ページ名**：`/guild` = マイ銀行
- **取引履歴セクション**：通帳（「通帳：これまでの お取引」）
- ボトムタブラベルは「マイ銀行」のまま維持
