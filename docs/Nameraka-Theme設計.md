...
1. **ヒーロー**：「AIエージェントで、」「あなたの時間を」「アップデート。」の 3 行強制改行（`<span className="block lg:inline">`、PC では 1 行）+ 「日本最大のAIエージェント・プラットフォーム」+ CTA **1 個**（「いま のこす」→ `/bank`）。価格チップ「いますぐ ¥30,000 から」は廃止済み。
2. **つかいかた はかんたん**（統合ブロック）：3 カード 3 列（📝のこす→/sell / 🤖AIが働く→/jobs / 💴¥が入る→/guild）、ピクト 36px + ラベル 16–18px + サブ 12px、矢印なし
3. **いま のこされた しごと**：横スクロール 3 件、タイトル＋¥価格のみ（説明文なし）
4. **かせげる しごと**：縦リスト 2 件、タイトル＋報酬＋「応募する」赤ボタン
5. **フッタ**：GUILD AI ロゴ＋ナビリンク（コピーテキスト最小）
