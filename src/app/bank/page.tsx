...
function ErrorToast({ msg }: { msg: string }) {
  return (
    <div
      role="alert"
      aria-live="assertive"
      className="fixed bottom-24 right-4 z-50 bg-[var(--primary,#6366F1)] text-white text-sm font-bold px-4 py-3 rounded-2xl shadow-lg animate-[slideInToast_220ms_ease-out]"
    >
      {msg}
    </div>
  );
    ...
    <main className="px-4 sm:px-6 lg:px-8 max-w-2xl mx-auto py-8">
      ...
      {step === "input" && (
        <div className="space-y-4">
          ...
          {inputMode === "file" && (
            <div
              ...
              className={`border-2 border-dashed rounded-2xl p-6 sm:p-10 flex flex-col items-center gap-4 transition-all duration-220 cursor-pointer ${
                isDragging
                  ? "border-[#6366F1] bg-red-50"
                  : "border-gray-300 bg-[var(--n-surface,#FFFFFF)] hover:border-[#6366F1] hover:bg-red-50/30"
              }`}
          ...
          {inputMode === "text" && (
            <div className="bg-[var(--n-surface,#FFFFFF)] border-2 border-dashed border-[var(--n-divider,rgba(0,0,0,0.12))] rounded-2xl p-4 hover:border-[var(--primary,#6366F1)]/40 transition-colors">
              {fileName && (
                <p className="text-xs text-[var(--primary,#6366F1)] font-semibold mb-2">📎 {fileName}</p>
              )}
              <textarea
              ...
              <div className="flex items-center justify-between mt-2 text-xs text-[var(--n-muted,#6B6456)]">
                <span>
                  ...
                  {remaining > 0 && <span className="ml-2 text-[var(--primary,#6366F1)]">（あと {remaining.toLocaleString("ja-JP")} 文字）</span>}
                </span>
                {noteContent.length > 50 && (
                  <span className="text-[var(--primary,#6366F1)] font-semibold">● Sランク候補</span>
                )}
              </div>
            </div>
          )}
          ...
          <button
            ...
            className="w-full py-3.5 rounded-full bg-[var(--primary,#6366F1)] text-white font-bold text-base disabled:opacity-30 disabled:cursor-not-allowed hover:bg-[#4F46E5] active:scale-[0.98] transition-all duration-220 shadow-md"
      ...
      {step === "scoring" && (
        <div className="bg-[var(--n-surface,#FFFFFF)] border border-[var(--n-divider,rgba(0,0,0,0.08))] rounded-3xl p-8 text-center relative overflow-hidden">
          <div className="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-transparent via-[var(--primary,#6366F1)] to-transparent animate-[scanLine_0.4s_ease-in-out_forwards]" aria-hidden />
          <p className="text-[var(--primary,#6366F1)] font-bold text-lg">鑑定中…</p>
      ...
      {step === "result" && auditResult && (
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={handleMint} className="flex-1 py-3 rounded-full bg-[var(--primary,#6366F1)] text-white font-bold hover:bg-[#4F46E5] active:scale-[0.98] transition-all">
              のこす →
            </button>
          </div>
        </div>
          ...
          <div className="bg-[var(--n-surface,#FFFFFF)] border border-[#4DD08F]/40 rounded-3xl p-8 text-center">
            ...
            <div className="flex gap-3 justify-center">
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-full bg-[var(--primary,#6366F1)] text-white font-bold hover:bg-[#4F46E5]"
              >
                かせぐ →
              </button>
            </div>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={handleCopy}
                  aria-label="URLをコピー"
                  className="shrink-0 px-3 py-1.5 rounded-xl bg-[var(--primary,#6366F1)] text-white text-xs font-bold hover:bg-[#4F46E5] active:scale-[0.98] transition-all focus:outline-none focus:ring-2 focus:ring-[var(--primary,#6366F1)] focus:ring-offset-1"
                >
                  {copied ? "コピー済み ✓" : "コピー"}
                </button>
              </div>
              ...
              <details open={showCurl}>
                <summary
                  className="text-[10px] text-[var(--primary,#6366F1)] font-semibold cursor-pointer select-none"
