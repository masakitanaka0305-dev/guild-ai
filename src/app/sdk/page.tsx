      ...
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-4">
                ...
                <li key={label} className="flex items-center gap-3">
                  <span
                    className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold transition-colors ${
                        ...
                        : active
                        ? "bg-[var(--primary,#4C1D95)] text-white animate-pulse"
          ...
          <button
            ...
            onClick={handleRun}
            disabled={running}
            className="w-full h-12 rounded-full bg-[var(--primary,#4C1D95)] text-white font-bold text-sm hover:opacity-90 active:scale-[0.98] transition-all disabled:opacity-50"
          >
    ...
    <main className="px-4 sm:px-6 lg:px-8 max-w-2xl mx-auto py-8 pb-24 sm:pb-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--n-muted,#6B6456)] leading-relaxed mb-1">
          ...
          ひとつの大きな<span className="text-[var(--primary,#4C1D95)] font-semibold">知能の集合体</span>として動作させる開発キット。
        </p>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2 mb-3">
          <div className="w-1 h-5 rounded-full bg-[var(--primary,#4C1D95)]" />
      ...
      <button
        ...
        className="w-full h-12 rounded-full bg-[var(--primary,#4C1D95)] text-white font-bold text-base hover:opacity-90 active:scale-[0.98] transition-all shadow-sm"
      >
        今すぐ試す
      </button>
        ...
        <ol className="flex flex-wrap gap-2">
          {PIPELINE_STEPS.map((step, i) => (
            <li key={step} className="flex items-center gap-1.5 text-xs text-[var(--n-text,#1A1714)]">
              <span className="w-4 h-4 rounded-full bg-[var(--primary,#4C1D95)] text-white text-[9px] flex items-center justify-center font-bold">{i+1}</span>
              {step}
