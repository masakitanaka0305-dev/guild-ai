  ...
  return (
    <main className="px-4 sm:px-6 lg:px-8 py-8 max-w-3xl mx-auto">
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        >
          <div className="w-10 h-10 rounded-xl bg-[var(--primary,#4C1D95)] flex items-center justify-center flex-shrink-0 text-white text-lg">
            📦
          </div>
      ...
      <div className="mt-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/feed/zero-day" className="text-xs text-[var(--primary,#4C1D95)] hover:underline font-semibold">
            すべて見る →
          </Link>
        </div>
          ...
          {events.slice(0, 3).map((event) => (
            <div key={event.id} className="section-card p-3 flex items-start gap-3">
              <span className={`mt-0.5 shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${
                event.priority === "critical" ? "bg-[var(--primary,#4C1D95)] text-white" :
      ...
      <div className="mt-8 section-card p-6 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link
            ...
            className="inline-block rounded-xl bg-[var(--primary,#4C1D95)] px-8 py-3 text-sm font-bold text-white hover:opacity-90 transition-opacity"
          >
            プランを選んで始める →
          </Link>
          <Link
