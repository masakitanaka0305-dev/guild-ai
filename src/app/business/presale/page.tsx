      ...
      <form onSubmit={handleSubmit} className="section-card p-5 space-y-4">
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="w-full rounded-xl border border-[var(--n-divider,rgba(0,0,0,0.12))] bg-[var(--n-surface-2,#F5F3EE)] px-4 py-3 text-sm text-[var(--n-text,#1A1714)] placeholder:text-[var(--n-muted,#6B6456)] focus:outline-none focus:ring-2 focus:ring-[var(--n-primary,#0000CC)] resize-none"
          />
        </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {EXAMPLE_QUERIES.map((q) => (
              <button
                ...
                type="button"
                onClick={() => setQuery(q)}
                className="text-[10px] px-2 py-1 rounded-full bg-[var(--n-surface-2,#F5F3EE)] text-[var(--n-muted,#6B6456)] border border-[var(--n-divider,rgba(0,0,0,0.08))] hover:border-[var(--n-primary,#0000CC)] hover:text-[var(--n-primary,#0000CC)] transition-colors"
              >
                {q}
              </button>
            ))}
        ...
        <button
          ...
          className="w-full rounded-xl bg-[var(--n-primary,#0000CC)] text-white text-sm font-bold py-3 hover:opacity-90 disabled:opacity-40 transition-opacity"
        >
          {loading ? "分析中..." : "提案を生成する →"}
        </button>
      </form>
      ...
      {proposal && !loading && (
        <div className="mt-6 space-y-4">
          ...
          {proposal.recommendedPackage && (
            <div className="section-card p-5 border-2 border-[var(--n-primary,#0000CC)]">
              <div className="flex items-start justify-between gap-2 mb-2">
                <h3 className="text-sm font-bold text-[var(--n-text,#1A1714)]">
                  推奨パッケージ: {proposal.recommendedPackage.title}
                </h3>
                <span className="shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--n-primary,#0000CC)] text-white">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <Link
              ...
              className="flex-1 text-center rounded-xl bg-[var(--n-primary,#0000CC)] px-6 py-3 text-sm font-bold text-white hover:opacity-90 transition-opacity"
            >
              このプランで申し込む →
            </Link>
            <Link
