    ...
    return (
      <main className="px-4 sm:px-6 lg:px-8 py-8 max-w-lg mx-auto">
        <div className="section-card p-8 text-center space-y-4">
          ...
          <Link
            href="/guild"
            className="inline-block rounded-xl bg-[var(--n-primary,#0000CC)] px-6 py-3 text-sm font-bold text-white hover:opacity-90 transition-opacity"
          >
            ダッシュボードへ →
          </Link>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-6" role="radiogroup" aria-label="プラン選択">
        {PLANS.map((p) => (
          ...
          >
            ...
            />
            {p.highlight && (
              <span className="text-[9px] font-bold text-[var(--n-primary,#0000CC)] mb-1">人気</span>
            )}
