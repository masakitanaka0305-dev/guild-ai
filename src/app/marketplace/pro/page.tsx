        ...
        {options.map((opt) => (
          <button
            ...
            className={`shrink-0 rounded-full border px-3 py-1 text-[10px] font-semibold transition-colors ${
              value === opt.value
                ? "bg-[var(--n-primary,#0000CC)] border-[var(--n-primary,#0000CC)] text-white"
                : "border-[var(--n-divider,rgba(0,0,0,0.08))] text-[var(--n-muted,#6B6456)] hover:border-[var(--n-primary,#0000CC)] hover:text-[var(--n-primary,#0000CC)]"
            }`}
          >
            {opt.label}
          </button>
...
function ResultCard({ result, rank }: { result: ProSearchResult; rank: number }) {
  const rankColors = { S: "#D4AF37", A: "#0000CC", B: "#9890A8" } as const;
  const rankColor = rankColors[result.rank] ?? "#9890A8";

  return (
    <li className="bg-white border border-[var(--n-divider,rgba(0,0,0,0.08))] rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row gap-4 items-start hover:border-[var(--n-primary,#0000CC)]/30 transition-colors">
      {/* Rank plate */}
      <div
      ...
      <div className="flex flex-row sm:flex-col gap-2 shrink-0 w-full sm:w-auto">
        <Link
          ...
          className="flex-1 sm:flex-none text-center h-9 px-4 rounded-full bg-[var(--n-primary,#0000CC)] text-white text-xs font-bold flex items-center justify-center hover:opacity-90 active:scale-[0.97] transition-all"
        >
          この知能で解決
        </Link>
        <a
    ...
    <main className="px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto py-8 pb-24 sm:pb-12">
      ...
      <form onSubmit={handleSearch} className="mb-6">
        ...
        <textarea
          ...
          className="w-full rounded-2xl border border-[var(--n-divider,rgba(0,0,0,0.08))] bg-[var(--n-surface,#FFFFFF)] px-4 py-3 text-sm text-[var(--n-text,#1A1714)] placeholder:text-[var(--n-muted,#6B6456)] focus:outline-none focus:ring-2 focus:ring-[var(--n-primary,#0000CC)] resize-none"
        />
        ...
        <div className="mt-4 flex items-center gap-3 flex-wrap">
          ...
          <button
            type="submit"
            className="ml-auto h-11 px-8 rounded-full bg-[var(--n-primary,#0000CC)] text-white font-bold text-sm hover:opacity-90 active:scale-[0.97] transition-all shadow-sm"
          >
            検索
          </button>
        </div>
