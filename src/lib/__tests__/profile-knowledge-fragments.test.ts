...
describe("/profile — Phase H layout", () => {
  it("rank badge sits at the top of the header at size 80 with the sub-label caption", () => {
    ...
    expect(src).toMatch(/data-testid="profile-header"[\s\S]{0,400}border-l-brand-primary/);
  });
  ...
  it("登記済み MD タブ が「知能の断片」カードを表示する（shell-gray + cyan border）", () => {
    ...
    // Each card uses bg-midnight-surface + border-l-brand-primary/30
    expect(src).toMatch(/data-testid="knowledge-fragment-card"[\s\S]{0,400}bg-midnight-surface/);
    expect(src).toMatch(/data-testid="knowledge-fragment-card"[\s\S]{0,400}border-l-brand-primary\/30/);
  });
});
