...
import { HallOfFameTicker } from "@/components/HallOfFameTicker";
...
function EnterpriseCTA() {
  return (
    <div
      ...
      className="lg:hidden flex-shrink-0 sticky bottom-16 z-20 bg-midnight-surface border-t border-brand-primary/20"
    >
      <Link
        href="/business/checkout"
        className="flex items-center justify-between px-4 py-2.5 text-[12px] font-semibold text-brand-primary underline-offset-4 hover:underline hover:bg-brand-primary/5 transition-colors"
      >
...
function EnterpriseCTADesktop() {
  return (
    ...
    >
      <Link
        href="/business/checkout"
        className="flex items-center gap-1.5 px-3 py-2 text-[11px] font-semibold text-brand-primary underline-offset-4 hover:underline hover:bg-brand-primary/5 transition-colors"
      >
        <span>🏢 提携・案件提供をご検討の企業様へ →</span>
      </Link>
      <Link
        ...
        className="px-3 py-2 text-[11px] font-semibold text-[#F59E0B]/90 underline-offset-4 hover:underline hover:bg-[#F59E0B]/5 transition-colors"
      >
        伝説 →
      </Link>
    </div>
    ...
    <>
      ...
      <aside className="hidden lg:flex w-52 flex-shrink-0 flex-col border-r border-[var(--n-divider,rgba(0,0,0,0.08))] bg-[#F2F0EB]">
        <div className="h-14 flex items-center gap-2.5 px-4 border-b border-[var(--n-divider,rgba(0,0,0,0.08))] flex-shrink-0">
          <div className="w-8 h-8 rounded-2xl flex items-center justify-center flex-shrink-0 bg-[var(--primary,#4C1D95)]">
