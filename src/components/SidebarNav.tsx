...
function TabIcon({ type, active }: { type: string; active: boolean }) {
  const cls = `w-5 h-5 ${active ? "stroke-[var(--n-primary,#0000CC)]" : "stroke-[var(--n-muted,#6B6456)]"}`;
  ...
  return (
    ...
    >
        ...
        return (
          <Link
            ...
            className={`w-full flex items-center gap-2.5 px-3 py-2 text-[13px] font-medium transition-all duration-220 active:scale-[0.98] rounded-2xl ${
              active
                ? "bg-[var(--n-primary,#0000CC)]/10 text-[var(--n-primary,#0000CC)] font-bold"
      ...
      <Link
        ...
        className={`w-full flex items-center gap-2.5 px-3 py-2.5 mt-1 text-[13px] font-bold rounded-2xl bg-[var(--n-primary,#0000CC)] text-white hover:bg-[#0000A8] active:scale-[0.98] transition-all duration-220 ${
  ...
  return (
    <div className="lg:hidden flex-shrink-0 relative">
      ...
      <Link
        href={PRIMARY_ACTION.href}
        aria-label="出す"
        className="absolute left-1/2 -translate-x-1/2 -top-6 z-10 w-14 h-14 rounded-full bg-[var(--n-primary,#0000CC)] text-white flex items-center justify-center shadow-xl text-2xl font-bold hover:bg-[#0000A8] active:scale-[0.95] transition-all duration-220"
      >
        ＋
      </Link>
      <nav
          ...
          return (
            <Link
              ...
              className={`relative flex flex-col items-center justify-center gap-0.5 py-1 active:scale-[0.97] transition-colors ${
                active ? "text-[var(--n-primary,#0000CC)]" : "text-[var(--n-muted,#6B6456)]"
              }`}
            >
              <TabIcon type={item.icon} active={active} />
              <span className="text-[10px] font-medium">{item.label}</span>
              {active && (
                <span className="absolute bottom-1 w-4 h-0.5 rounded-full bg-[var(--n-primary,#0000CC)]" />
              )}
            </Link>
          );
        })}
