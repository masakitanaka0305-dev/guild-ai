...
function TabIcon({ type, active }: { type: string; active: boolean }) {
  const cls = `w-5 h-5 ${active ? "stroke-brand-primary" : "stroke-slate-400"}`;
  ...
  return (
    <div className="lg:hidden flex-shrink-0 relative z-40 pb-[env(safe-area-inset-bottom)]">
      ...
      <Link
        ...
        className="absolute left-1/2 -translate-x-1/2 -top-6 z-30 min-w-[44px] min-h-[44px] w-14 h-14 rounded-full bg-brand-primary text-text-on-primary flex items-center justify-center shadow-xl text-2xl font-bold hover:bg-brand-primary active:scale-[0.95] transition-all duration-220"
      >
        ＋
      </Link>
      <nav
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <span aria-hidden className="absolute bottom-0 left-2 right-2 h-0.5 rounded-full bg-brand-primary" />
              )}
            </Link>
          );
        })}
