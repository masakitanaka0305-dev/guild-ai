...
function Hi({ children }: { children: React.ReactNode }) {
  return (
    <span className="text-[#0000CC] font-semibold">{children}</span>
  );
}
    ...
    >
      ...
      >
        ...
        <button
          ...
          onClick={onClose}
          aria-label="閉じる"
          className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--n-primary,#0000CC)]"
        >
          ✕
        </button>
        ...
        <Link
          href="/sell"
          onClick={onClose}
          className="block w-full h-12 rounded-full bg-[var(--n-primary,#0000CC)] text-white font-bold text-base text-center leading-[3rem] hover:bg-[#0000A8] active:scale-[0.98] transition-all duration-220 shadow-sm"
        >
          今すぐ投稿する
        </Link>
      </div>
